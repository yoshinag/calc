...
let codeInput, graphOutput, notationRadios, templateSelect, downloadBtn,
    ...
    controlsPanel, toggleControlsBtn, fullscreenBtn; // ★fullscreenBtn を追加

// アプリケーションの状態変数
let vizInstance = null;
let currentNotation = 'mermaid';
...
function initializeApp() {
    ...
    // ★★★★★ fullscreenBtn のイベントリスナーをここに追加 ★★★★★
    fullscreenBtn.addEventListener('click', toggleFullScreenHandler);
    ...
    if (graphOutput.innerHTML.includes('<p>ライブラリを読み込み中です...</p>')) {
...
function setInitialPaneHeights() {
    if (!contentArea || !splitter || !editorPane || !previewPane) return;
...
function toggleFullScreenHandler() {
    if (!document.fullscreenEnabled && !document.webkitFullscreenEnabled && !document.mozFullScreenEnabled && !document.msFullscreenEnabled) {
        alert("お使いのブラウザでは全画面表示がサポートされていません。");
        return;
    }

    const elementToFullscreen = graphOutput; // graph-output を全画面にする
...
function handleFullscreenChange() {
    if (isFullScreen()) {
        ...
        fullscreenBtn.title = "通常表示に戻す";
        // 全画面時に特定のスタイルを適用する場合 (例: #graph-output の padding をなくすなど)
        // graphOutput.classList.add('fullscreen-active');
    } else {
        fullscreenBtn.textContent = "全画面";
        fullscreenBtn.title = "全画面表示";
        // graphOutput.classList.remove('fullscreen-active');
    }
}
// --- ★★★★★ 全画面表示ロジックここまで ★★★★★ ---
...
window.addEventListener('load', () => {
    ...
    fullscreenBtn = document.getElementById('fullscreen-btn'); // ★fullscreenBtn を取得
